...
public class SearchByCountry extends AppCompatActivity {
    ...
    static ArrayList<JSONObject> countriesList = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        RequestQueue volleyQueue = Volley.newRequestQueue(SearchByCountry.this);
        ...
        JsonObjectRequest jsonObjReq = new JsonObjectRequest(Request.Method.GET,
                ...
                (Response.Listener<JSONObject>) response ->{
                    try {
                        JSONArray jaFilms = response.getJSONArray("countries");

                        for(int i=0;i<jaFilms.length();i++){
                            countriesList.add((jaFilms.getJSONObject(i)));
                        }

                        ListView listView = findViewById(R.id.lvSBC);
                ...
                (Response.ErrorListener) error->{
                    Toast.makeText(SearchByCountry.this, error.getLocalizedMessage(), Toast.LENGTH_LONG).show();
                })
        {
            @Override
